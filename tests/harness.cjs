...
function harness(options = {}) {
    ...
    source = source.replaceAll('import.meta', '({url: "http://localhost/index.js"})');
    ...
    source=source.slice(0,boundary)+`
        ...
        globalThis.testApi = {readCompletionBody, safeParseJsonResponse, decompressZstd, getSettings, persistSettings, getChatState, activateEvent, buildEventPrompt, buildTurnGuidance, buildDynamicSlipStructure, buildRecentContext, cleanRecentContext, generateAndSave, askLLM, findTriggeredEvent, buildActiveStagePrompt, bindSTEvents, EventInjectionTool, refreshWorldInfoCache, buildWorldInfoSystemPrompt, DEFAULT_SETTINGS, DEFAULT_PRESETS, DEFAULT_SUB_PROMPTS, DEFAULT_JAILBREAK_PROMPT, DEFAULT_NOVEL_BYPASS_PROMPT, EVENT_TYPES, replanFromCurrent, stageGroupForTurn, splitEvenly, collectSettingsForm, handleCapsuleAction, tryAdvanceActiveEvent, setActiveEventById, stopActiveEvent, deleteEventById, sendEventTrigger, isMainGenerationBusy};
    })();`;
